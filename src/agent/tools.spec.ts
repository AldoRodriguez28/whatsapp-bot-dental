...
describe('agent tools', () => {
  ...
  it('get_availability devuelve horarios formateados', async () => {
      ...
      getAvailability: jest.fn().mockResolvedValue([
        {
          ...
          endsAt: new Date('2026-06-29T15:30:00Z'),
        },
      ]),
    } as any;
    ...
    expect(scheduling.getAvailability).toHaveBeenCalledWith(
      ctx.clinic,
      '2026-06-29',
    );
  ...
  it('book_appointment reporta slot ocupado', async () => {
    const scheduling = {
      bookAppointment: jest
        .fn()
        .mockResolvedValue({ ok: false, reason: 'slot_taken' }),
    } as any;
    const out = await executeTool(
      'book_appointment',
      {
  ...
  it('book_appointment rechaza dateTime no-ISO con Error', async () => {
    const out = await executeTool(
      'book_appointment',
      {
        patientName: 'Ana',
        treatment: 'limpieza',
        dateTime: 'mañana a las 3pm',
      },
      ctx,
      {} as any,
    );
