...
function make() {
  ...
  const scheduling = {
    ...
    confirmAppointment: jest.fn().mockResolvedValue({ ok: true }),
  } as any;
...
describe('RouterService', () => {
  ...
  it('botón cancel_<id> cancela la cita con el appointmentId', async () => {
    const { router, scheduling, whatsapp } = make();
    await router.handle(clinic, '521', 'cancel_appt9');
    expect(scheduling.cancelAppointment).toHaveBeenCalledWith(clinic, {
      phone: '521',
      appointmentId: 'appt9',
    });
    expect(whatsapp.sendText).toHaveBeenCalled();
  ...
  it('botón confirm_<id> confirma y responde en español', async () => {
    const { router, whatsapp, scheduling } = make();
    await router.handle(clinic, '521', 'confirm_appt9');
    expect(scheduling.confirmAppointment).toHaveBeenCalledWith(clinic, 'appt9');
    expect(whatsapp.sendText).toHaveBeenCalledWith(
      expect.anything(),
      '521',
